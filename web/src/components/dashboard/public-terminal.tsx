...
function derive(logs: string[]) {
  ...
  for (const raw of logs) {
    ...
    // API mode steps
    else if (msg.includes("API mode"))        { t.task = "API Mode"; t.detail = ""; }
    else if (msg.includes("Step 0"))          { t.task = "Establishing session"; t.detail = ""; }
    else if (msg.includes("Step 1"))          { t.task = "Getting captcha"; t.detail = ""; }
    else if (msg.includes("Solving captcha")) { t.task = "Solving captcha"; t.detail = ""; }
    else if (msg.includes("Register+icode"))  { t.task = "Submitting captcha"; t.detail = ""; }
    else if (msg.includes("Captcha solution")) { t.task = "Captcha solved"; t.detail = msg.split(":").pop()?.trim() || "✓"; }
    else if (msg.includes("apiKey:✓") || msg.includes("API key created")) { t.status = "success"; t.task = "Done"; t.threadDone++; t.detail = "success"; }
    else if (msg.includes("apiKey:✗") || msg.includes("passToken:✗")) { t.status = "failed"; t.task = "Failed"; t.threadDone++; t.detail = "no key"; }
    else if (msg.includes("Done in"))         { t.task = "Finished"; t.detail = msg; }
  }
...
export function PublicTerminal({ logs, batchId, onClose }: Props) {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.3, ease: [0.25, 0.1, 0.25, 1], opacity: { duration: 0.2 } }}
      className="overflow-hidden"
    >
      <div className={`border border-border/40 rounded-xl overflow-hidden bg-[#0a0a0a] shadow-2xl shadow-black/40 ${containerClass}`}>
        ...
        <div ref={ref} onScroll={handleScroll}
          className={`overflow-y-auto ${expanded ? "flex-1 min-h-0" : "max-h-[60vh] sm:max-h-[500px]"}`}>
              ...
              {threads.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
                  {threads.map(t => {
                    ...
                    return (
                      <div key={t.id} className={`rounded-lg border ${ac.border} ${ac.bg} p-2.5 sm:p-3 transition-all duration-300 ease-out min-w-0`}>
                        ...
                        <div className={`text-xs sm:text-sm font-medium truncate transition-colors duration-300 ${t.status === "success" ? "text-emerald-400" : t.status === "failed" ? "text-red-400" : "text-zinc-300"}`}>{t.task}</div>
                        {t.detail && <div className="text-[10px] text-zinc-500 font-mono mt-0.5 truncate">{t.detail}</div>}
                        {t.total > 0 && (
                          <div className="mt-2 h-1 bg-white/[0.04] rounded-full overflow-hidden">
                            <div className={`h-full rounded-full transition-all duration-700 ease-out ${t.status === "success" ? "bg-emerald-500/60" : t.status === "failed" ? "bg-red-500/60" : ac.bar + "/60"}`}
